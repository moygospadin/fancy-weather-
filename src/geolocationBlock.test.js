...
describe('Correct city', () => {
  ...
  test('Washington', async () => {
    expect((await getСityСoordinates('Washington', 'en', 0)).results[0].components.country).toBe('USA');
  });
  test('New York', async () => {
    expect((await getСityСoordinates('New York', 'en', 0)).results[0].components.country).toBe('USA');
  });
  test('Bern', async () => {
